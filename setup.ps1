...
# ---------------------------------------------------------------------------
Write-Header "Setup Mode"
Write-Info "Choose your setup path:"
Write-Host ""
Write-Host "  [1] Full setup    — configure repos, services, and integrations" -ForegroundColor White
Write-Host "  [2] Demo mode     — minimal config, pair with Drone to see everything" -ForegroundColor White
Write-Host ""
$SetupMode = Ask-Question "Choose (1 or 2)" "1"

if ($SetupMode -eq "2") {
    Write-Header "Demo Mode"
    Write-Info "Generating minimal config for demo use..."

    $DefaultProjectsDir = Split-Path -Parent $ScriptDir
    $ProjectsDir = $DefaultProjectsDir

    # Auto-detect drone sibling
    $DroneSibling = Join-Path $DefaultProjectsDir 'drone'
    $DemoRepos = @("hive")
    if (Test-Path $DroneSibling) {
        $DemoRepos += "drone"
        Write-Success "Found drone at: $DroneSibling"
    } else {
        Write-Warn "Drone not found at $DroneSibling — clone it there for the full demo experience."
    }
    $ReposJson = Build-JsonArray $DemoRepos
...
"@
    ...
    Write-Success "Created dashboard.config.json (demo mode)"

    # Create data directory and empty files
    $DataDir = Join-Path $ScriptDir 'data'
    if (-not (Test-Path $DataDir)) { New-Item -ItemType Directory -Path $DataDir | Out-Null }
    Set-Content -Path $DatabasesFile -Value '[]' -Encoding UTF8
    Write-Info "Created data/databases.json (empty)"

    # npm install (hive)
    Write-Host ""
    Write-Info "Installing HIVE dependencies..."
    Set-Location $ScriptDir
    npm install
    Write-Success "HIVE dependencies installed"

    # npm install (drone — if detected)
    $DroneReady = $false
    if (Test-Path $DroneSibling) {
        Write-Host ""
        Write-Info "Installing Drone dependencies..."
        Set-Location $DroneSibling
        npm install
        Write-Success "Drone dependencies installed"
        Set-Location $ScriptDir
        $DroneReady = $true
    }
    ...
    $gitName  = if ((git config --global user.name 2>$null))  { git config --global user.name }  else { "Demo User" }
    $gitEmail = if ((git config --global user.email 2>$null)) { git config --global user.email } else { "demo@example.com" }

    $SharedContent = @"
# Hivemind Config
# Generated by H.I.V.E. setup (demo mode) — re-run hive/setup.ps1 to update.
...
"@
    ...
    Write-Host "  H.I.V.E. demo setup complete!" -ForegroundColor Green
    Write-Host ("=" * 62) -ForegroundColor Green
    Write-Host ""

    if (-not $DroneReady) {
        Write-Host "  Drone was not found. Clone it as a sibling and re-run:" -ForegroundColor Yellow
        Write-Host "    git clone <drone-repo-url> $DroneSibling" -ForegroundColor Cyan
        Write-Host "    .\setup.ps1   (choose Demo mode again)" -ForegroundColor Cyan
    } else {
        Write-Host "  Both HIVE and Drone are ready. Open two terminals:" -ForegroundColor White
        Write-Host ""
        Write-Host "    Terminal 1 (HIVE):" -ForegroundColor Cyan
        Write-Host "      cd $ScriptDir" -ForegroundColor White
        Write-Host "      npm start" -ForegroundColor White
        Write-Host ""
        Write-Host "    Terminal 2 (Drone):" -ForegroundColor Cyan
        Write-Host "      cd $DroneSibling" -ForegroundColor White
        Write-Host "      npm start" -ForegroundColor White
        Write-Host ""
        Write-Host "    Drone auto-detects HIVE and configures it on startup." -ForegroundColor Green
        Write-Host "    Start order doesn't matter — drone retries until HIVE is up." -ForegroundColor Green
        Write-Host ""
        Write-Host "    HIVE:           http://localhost:3333" -ForegroundColor Cyan
        Write-Host "    Drone Control:  http://localhost:4000" -ForegroundColor Cyan
    }
    Write-Host ""
    exit 0
}
...
Write-Host "    2. Add to your PowerShell profile (" -ForegroundColor White -NoNewline
Write-Host '$PROFILE' -ForegroundColor Cyan -NoNewline
Write-Host "):" -ForegroundColor White
Write-Host "       function claude { & claude.exe --add-dir '$ProjectsDir\hivemind' @args }" -ForegroundColor Cyan
Write-Host ""
